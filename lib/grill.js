...
const sendCommand = (address, port, command, callback) => {
  ...
  //   setTimeout(function(){
  //     socket.close();
  //     throw new Error('connection to grill timed out')
  //   },1500);
  // });
...
const readBuf = (bytes) => {
    ...
    let data = {
      temp: buffer.readUInt8(grillTemp)+(buffer.readUInt8(grillTempHigh)*256),
      tempSet: buffer.readUInt8(grillSetTemp)+(buffer.readUInt8(grillSetTempHigh)*256) ,
      probe: [ 
        {
          temp: buffer.readUInt8(probeTemp)+(buffer.readUInt8(probeTempHigh)*256), 
...
class greenMountainGrill{
  ...
  temp(device, temperature, callback){
    ...
    let port = this.port;
    /** Validate parameters */
    const hasDeviceProperty = Object.prototype.hasOwnProperty.call(commands.tempSet, device);
